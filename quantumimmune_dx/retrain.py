...
print(f"Unique classes: {list(label_encoder.classes_)}")
...
# Compute quantum kernel matrix (state fidelity)
...
def compute_kernel_matrix(X):
    ...
    for i in range(n_samples):
        for j in range(i, n_samples):
            val = kernel_fidelity(X[i], X[j])
            K[i, j] = val
            K[j, i] = val
            count += 1
            if count % 50 == 0:
                elapsed = time.time() - start_time
                print(f"Computed {count}/{total_pairs} pairs ({elapsed:.1f}s)")
    elapsed = time.time() - start_time
    print(f"Kernel matrix computation completed in {elapsed:.1f}s")
    return K
...
# Save the kernel matrix? Not required, but we can save it for inspection
# joblib.dump(K, 'models/qsvm_kernel.joblib')
